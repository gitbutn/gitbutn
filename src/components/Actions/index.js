...
const button = list => {
  let urlw = `${url}/button/?&title=${list.title}&sub=${list.sub}&icon=${
    list.icon
  }&ico=${list.ico}&lco=${list.lco}&rco=${list.rco}&tc=${list.tc}&sc=${
    list.sc
  }&size=${list.size}`;
...
const badge = list => {
  const isThereIcon =
    list.icon !== undefined && list.icon.length > 2
      ? `&icon=${list.icon}&ico=${list.ltc}`
      : "";
  let uri = `${url}/badge/?&title=${list.title}&sub=${
    list.sub
  }${isThereIcon}&lco=${list.lco}&rco=${list.rco}&ltc=${list.ltc}&rtc=${
    list.rtc
  }&ver=${list.ver}`;
  return new Promise((res, rej) => {
    fetch("https://u.gitbutn.io/new", {
